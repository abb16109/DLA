...
```{r range}
preddat2 <- expand.grid((rangeMAP = seq(200,1600, 200)), pathogenorigin = scale(quantile(dat$pathogenorigin, probs = c(0.1,0.5,0.9))))
...
#preddat1$preds <- predict(rangeMAPmod, newdata = preddat1, re.form = ~0)
#preddat2$preds <- predict(rangeMAPmod, newdata = preddat1, re.form = ~0)
...
pred.fun2 <- function(.) {
 preddat2 <- expand.grid(hostorigin = seq(1800,3000, 200), 
...
preddat2<- data.frame(preddat2, confint(meanMAP.boot))
...
rangeplot <- preddat2 %>% ggplot(aes(x=Var1, y=preds)) +
